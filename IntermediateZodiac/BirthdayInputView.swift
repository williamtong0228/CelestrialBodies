...
struct BirthdayInputView: View {
    ...
    @State var showSecInputView = false
    ...
    var body: some View {
        VStack{
            ...
            Button("Compute Chart") {
                sharing.julianCentury = Declination.julianCentury(Declination.date(toJulian: reconstructedTime))
                sharing.newDate = reconstructedTime
                
                positions.baseWheel = ChartViewModel.createWheel(date: reconstructedTime)
                sharing.nineGeoCoordinates = positions.baseWheel.wheelPos[0].nineGeoLatLon
                sharing.planetsPos = positions.DrawWheel(meanLong: sharing.nineGeoCoordinates, julianCentury: sharing.julianCentury, ringRadius: innerRing)
                popSheet = true
            }
            ...
            .sheet(isPresented: $showSecInputView) {
                SecWheelInputView()
            }
        }
